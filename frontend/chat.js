...
async function Group()
{
   ...
   dom2=setInterval(async (e) => {
    try{
        await axios.get('http://13.232.243.73:5000/get/groups',{headers:{'token':token}})
        .then(res=>{
...
function groInt(data)
{
    gMessg=setInterval(async (e) => {
        try{
            await  axios.get('http://13.232.243.73:5000/group/getM',{headers:{'token':token,'group':data}})
          .then(res=>{
              console.log(res.data);
              Gchat.innerHTML='';
...
function inter()
{
   dom= setInterval(async (e) => {
    try{
     await axios.get('http://13.232.243.73:5000/users',{headers:{'token':token}})
     .then(res=>{
        console.log(res.data);
        u.innerHTML='';
        show(res.data.obj);
    ...
    for(var i=0;i<user.length;i++)
    {
        user[i].addEventListener('click',async (e)=>{
            ...
            let n=e.target.innerText;
                try{
                    await axios.get('http://13.232.243.73:5000/user/chats',{headers:{'token':token,'secondUser':n}})
                    .then(res=>{
                       console.log(res.data);
                    ...
                    messg =  setInterval(() => {
                        ...
                        axios.get('http://13.232.243.73:5000/chats',{headers:{'logger':l,'secondary':s}})
                        .then(res=>{
                            console.log(res.data);
                            chatSec.innerHTML='';
                            for(var i=0;i<res.data.length;i++)
...
send.addEventListener('click',async (e)=>{
    ...
    let obj={mssg,lo,su};
    try{
        await axios.post('http://13.232.243.73:5000/user/addchat',obj)
        .then(res=>{
            console.log(res.data);
...
create.addEventListener('click',(e)=>{
    ...
    let obj={token,gName};
    axios.post('http://13.232.243.73:5000/create/group',obj)
    .then(res=>{
        console.log(res.data);
        Gchat.innerHTML='';
        gChat.classList.toggle('active');
...
addP.addEventListener('click',async (e)=>{
    ...
    let obj={gName,par};
    try{
       await axios.post('http://13.232.243.73:5000/group/addp',obj).then(res=>{
        console.log(res.data);
...
gmessge.addEventListener('click',async (e)=>{
    ...
    try{
       await axios.post('http://13.232.243.73:5000/group/addM',obj)
       .then(r=>{
        console.log(r.data);
...
aParticipants.addEventListener('click',async (e)=>{
   ...
   let gN=f.id;
   try{
    await axios.get('http://13.232.243.73:5000/group/memN',{headers:{'token':token,'group':gN}})
    .then(res=>{
        console.log(res.data);
        allMembers(res.data);
    })
...
function remAdmin(data)
{
    for(var i=0;i<data.length;i++)
    {
     data[i].addEventListener('click',async (e)=>{
         try{
             await axios.get(`http://13.232.243.73:5000/group/Radmin`,{headers:{'id':e.target.id,'group':n.innerText,'token':token}})
             .then(res=>{
                 console.log(res.data);
                 popup('admin removed');
...
function MakeAd(data)
{
    for(var i=0;i<data.length;i++)
    {
     data[i].addEventListener('click',async (e)=>{
         try{
             await axios.get(`http://13.232.243.73:5000/group/admin`,{headers:{'id':e.target.id,'group':n.innerText,'token':token}})
             .then(res=>{
                 console.log(res.data);
                 popup(res.data.message);
...
function remove(rem)
{
   for(var i=0;i<rem.length;i++)
   {
    rem[i].addEventListener('click',async (e)=>{
        try{
            await axios.get(`http://13.232.243.73:5000/group/member`,{headers:{'id':e.target.id,'group':n.innerText}})
            .then(res=>{
                console.log(res.data);
...
delGroup.addEventListener('click',async(e)=>{
    try{
        await axios.delete(`http://13.232.243.73:5000/group/del/${n1.innerText}`)
        .then(res=>{
            console.log(res.data);
            popup(res.data.message);
            GroupContainr.classList.remove('active');
...
b.addEventListener('click',async(e)=>{
    try{
        await axios.get(`http://13.232.243.73:5000/group/leave`,{headers:{'token':token,'group':n1.innerText}})
        .then(res=>{
            console.log(res.data);
            popup(res.data.message);
            gB();
